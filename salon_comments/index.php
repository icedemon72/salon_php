<?php
  // --- POCETNA STRANICA ---
  // obavezno pogledati i html kod jer se i u njemu nalazi PHP delovi

  // session_start() oznacava pocetak sesije, u sesiji se cuvaju kolacici tj. promenljive
  // koje se pamte kroz razlicite stranice, npr. kad se ulogujemo postavljamo sesijsku promenljivu
  // korisnickoIme, email isl.  koje kasnije mozemo da koristimo i na drugom delu sajta radi verifikacije npr.
  session_start();

  // ukljucujemo $conn promenljivu iz db.php koja sluzi za pristup bazi podataka
  include ("./db/db.php");

  // IZABERI SVE IZ kategorije
  $sql = "SELECT * FROM kategorije";

  // skladisti rezultat upita u $result
  $result = $conn->query($sql);

  // ovo je asocijativni niz u PHP-u, on ide u paru key=>value, tj svaki "kljuc" ima sebi dodeljenu vrednost
  // u ovom slucaju, asocijativni niz $categories, cuva druge nizove $id, $naziv... gde cemo smestiti
  // informacije o svakoj kategoriji
  // znamo da pripadaju istoj kategoriji na osnovu njihove pozicije, 
  // npr kategoriji sa id-em $categories['id'][0] odgovarace naziv sa nazivom $categories['naziv'][0] itd.
  $categories = array(
    'id' => array(),
    'naziv' => array(),
    'slika' => array()
  ...
  // u promenljivu $row skladistimo jedan red iz baze koji odgovara nasem upitu $sql (linija 14)
  // zatim vrsimo iteraciju kroz svako poklapanje i push-ujemo (guramo, ubacujemo) vrednosti u $categories niz
...
<body>
    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        ...
        <div class="d-lg-flex d-xs-block mx-lg-5 gap-3 nav-item">
          <!-- Ukoliko ne postoje sesijske promenljiva 'user' i 'admin' (znaci da niko nije ulogovan)  -->
          <?php if(!isset($_SESSION["user"]) && !isset($_SESSION["admin"])): ?>
            <!-- Prikazi ovo: -->
            <a class="nav-link" href="./pages/login.php">Prijava</a>
          <!-- U suprotnom (neko je ulogovan) -->
          <?php else: ?>
            <!-- Proveri da li postoji sesijska promenljiva 'admin' -->
            <?php if(isset($_SESSION["admin"])): ?>
              <!-- Ukoliko postoji prikazi ovo -->
            ...
            <?php endif; ?>
            <!-- Ovo (odjavi se) prikazi nevezano da li je ulogovan admin ili korisnik -->
  ...
  <section id="front" class="d-flex align-items-center justify-content-center">
    <div class="container position-relative">
      ...
      <div class="w-100 d-flex justify-content-center">
        <!-- 
          link ovog dugmeta se odredjuje na osnovu sesijske promenljive, ako korisnik nije ulogovan
          onda klikom na njega salje se na login stranicu, u suprotnom posalji ga na #usluge div
        -->
        <?php if(!isset($_SESSION['user']) && !isset($_SESSION["admin"])): ?>
  ...
  <section id="o_nama" class="mt-5 p-5">
    <div class="container">
      <div class="row">
        ...
        <!-- Ovo je lorem ipsum tekst, ukoliko zelis mozes da dodas svoj tekst -->
  ...
  <section id="usluge" class="mt-5 p-5">
    <div class="container">
      ...
      <div class="row">
        <!-- 
          Ovo je for loop u ovom formatu i ide od 0 do duzine $categories["id"],
          tj. ide od 0 do broja kategorija, tkd pomocu njega mi ispisujemo sve informacije
          o kategorijama na dnu stranice
        -->
